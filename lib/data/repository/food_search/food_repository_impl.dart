...
import '../../datasource/remote/food_search_api.dart';
...
class FoodRepositoryImpl implements FoodRepository {
  ...
  @override
  Future<List<DailyNutrition>> getFoodNutrition(String foodName) async {
  ...
  List<DailyNutrition> _parseFoodNutrition(String responseBody, String foodName) {
    // foodName 파라미터 추가
    try {
      ...
      if (foodsData == null) {
        ...
        debugPrint('_parseFoodNutrition: No "foods" key in response for foodName: $foodName'); // 로그 추가
        return []; // 빈 리스트 반환
      }
      ...
      if (foodList == null || foodList.isEmpty) {
        ...
        debugPrint('_parseFoodNutrition: No food data found for foodName: $foodName'); // 로그 추가
        return []; // 빈 리스트 반환
      }

      List<DailyNutrition> foodNutritions = [];
      ...
      debugPrint('_parseFoodNutrition: JSON parsing error for foodName: $foodName, error: $e'); // 로그 추가 (foodName 포함)
